...
@LastEditTime: 2019-07-10 21:35:10
...
class Model(object):
    ...
    def get_cosine(self,q,a,name):
        ...
        if self.dropout_keep_prob_holder != 1.0:
            ...
            tf.sqrt(tf.reduce_sum(pooled_flat_1*pooled_flat_1, 1)) * tf.sqrt(tf.reduce_sum(pooled_flat_2*pooled_flat_2, 1)) + 1e-8,
            name="cosine")
        ...
        #     a_normalize = tf.nn.l2_normalize(a, dim=1)
            ...
            cosine = tf.div(
                tf.reduce_sum(q*a, 1),
                tf.sqrt(tf.reduce_sum(q*q, 1)) * tf.sqrt(tf.reduce_sum(a*a, 1)) + 1e-8,
                name="cosine")
    ...
    def predict(self, data_batch):
        ...
        for data in data_batch:
            ...
            score = self.sess.run(self.score12, feed_dict)
            scores.extend(score)
        return scores
